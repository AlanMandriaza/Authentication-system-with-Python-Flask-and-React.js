...
const getState = ({ getStore, setStore }) => {
  ...
  return {
    ...
    actions: {
      ...
      addToFavorite: (item) => {
        const store = getStore();
        const itemInFavorite = store.favorite.find((i) => i.id === item.id);

        if (itemInFavorite) {
          const newFavorite = store.favorite.map((i) =>
            i.id === item.id ? { ...i, quantity: i.quantity + 1 } : i
          );
          const newState = { ...store, favorite: newFavorite };
          setStore(newState);
          setStateToLocalStorage(newState);
        } else {
          const newFavorite = [...store.favorite, { ...item, quantity: 1 }];
          const newState = { ...store, favorite: newFavorite };
          setStore(newState);
          setStateToLocalStorage(newState);
        }
      ...
      removeFromFavorite: (itemIndex) => {
        const store = getStore();
        const newFavorite = [...store.favorite];
